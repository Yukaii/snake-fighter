...
class LocalGame {
  ...
  isPositionFree(x, y, excludeSnakes = false) {
    ...
    if (!excludeSnakes) {
      if (this.player1.alive && this.player1.snake.some((segment) => segment.x === x && segment.y === y)) {
        return false;
      }
      if (this.player2.alive && this.player2.snake.some((segment) => segment.x === x && segment.y === y)) {
        return false;
      }
    }
    return true;
  ...
  _checkCollisionForPlayer(player, otherPlayer) {
    ...
    if (otherPlayer.alive && otherPlayer.snake.some((segment) => segment.x === head.x && segment.y === head.y)) {
  ...
  _calculateAIDirection() {
    ...
    const isPositionSafe = (pos) => {
      if (
        pos.x < 0 || pos.x >= this.config.CANVAS_WIDTH ||
      ...
      if (humanPlayer.alive && humanPlayer.snake.some((seg) => seg.x === pos.x && seg.y === pos.y)) return false;
      return true;
    };
    ...
    const calculateAvailableSpace = (startPos) => {
      ...
      while (queue.length > 0 && spaceCount < 50) { // Limit search depth
    ...
    const directionScores = validDirections.map((direction) => {
      ...
      if (!isPositionSafe(nextHead)) {
        ...
        for (let lookahead = 1; lookahead <= 5; lookahead++) {
          ...
          if (!isPositionSafe(futureHead)) {
           ...
           const wallDistance = Math.min(
            futureHead.x, this.config.CANVAS_WIDTH - futureHead.x - this.config.GRID_SIZE,
            futureHead.y, this.config.CANVAS_HEIGHT - futureHead.y - this.config.GRID_SIZE
          );
        ...
        if (this.seeds.length > 0) {
          ...
          if (availableSpace > 10) { // Only seek seeds if space allows
  ...
  update() {
    ...
    if (this.player2.isAI && this.player2.alive) {
      ...
      if (this.canPlaceObstacle(2) && Math.random() < 0.05) { // Reduced chance
    ...
    if (alivePlayers.length <= (this.isAIMode ? 0 : 1)) { // If AI mode, game over if human dies. If 2P, if 1 or 0 alive.
