defmodule SnakeFighterServerWeb.SocketIOHandler do
  ...
  @doc """
  Registers a WebSocket process for a session
  """
  def register_websocket_process(session_id, pid) do
    GenServer.call(__MODULE__, {:register_websocket, session_id, pid})
  end

  @doc """
  Sends noop packet to polling transport during WebSocket upgrade
  """
  def send_noop_to_polling(session_id) do
    GenServer.cast(__MODULE__, {:send_noop, session_id})
  end
  ...
  @impl true
  def handle_call({:register_websocket, session_id, pid}, _from, state) do
    new_websocket_processes = Map.put(state.websocket_processes, session_id, pid)
    new_state = %{state | websocket_processes: new_websocket_processes}
    {:reply, :ok, new_state}
  end

  @impl true
  def handle_cast({:send_noop, session_id}, state) do
    # Send noop packet to close any pending polling requests
    alias SnakeFighterServer.EngineIO
    noop_packet = EngineIO.encode_packet(EngineIO.packet_types().noop)
    new_state = send_packet_to_session(session_id, noop_packet, state)
    {:noreply, new_state}
  end
  ...
  defp handle_socket_io_packet(session_id, packet, state) do
    ...
    case packet.type do
      type when type == socket_types.connect ->
        # Handle Socket.IO connect packet
        Logger.debug("Socket.IO connect packet received for session: #{session_id}")
        # Mark session as connected and send connect response with session ID
        case Map.get(state.sessions, session_id) do
          nil -> state
          socket ->
            updated_socket = %{socket | connected: true}
            new_sessions = Map.put(state.sessions, session_id, updated_socket)

            # Send connect acknowledgment with session ID (Socket.IO v5 requirement)
            connect_data = %{sid: socket.engine_session.session_id}
            connect_ack = SocketIO.encode_packet(socket_types.connect, packet.namespace, connect_data)
            new_state = %{state | sessions: new_sessions}
            
            # IMPORTANT: Use the return value from send_packet_to_session
            send_packet_to_session(session_id, connect_ack, new_state)
        end
      ...
      type when type == socket_types.disconnect ->
        # Handle disconnect
        Logger.debug("Socket.IO disconnect packet received for session: #{session_id}")
        state
