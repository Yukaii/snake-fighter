defmodule SnakeFighterServer.SocketIO do
  ...
  @doc """
  Handles Socket.IO handshake (Engine.IO handshake + Socket.IO connect)
  """
  def handshake(%__MODULE__{} = socket) do
    engine_handshake = EngineIO.handshake_response(socket.engine_session)
    connect_packet = encode_packet(@packet_connect, socket.namespace)
    
    {[engine_handshake, connect_packet], %{socket | connected: true}}
  end
  ...
  def encode_packet(type, namespace, data, id) when type in 0..6 do
    ...
    packet_str = if namespace != @default_namespace do
    ...
    EngineIO.encode_packet(EngineIO.packet_types().message, packet_str)
  end
  ...
  defp parse_socket_packet(packet_data) when is_binary(packet_data) do
    case packet_data do
      <<type_char, rest::binary>> when type_char in ?0..?6 ->
        ...
        # Parse namespace, ack_id, and data
  ...
  defp parse_namespace(rest), do: {@default_namespace, rest}
