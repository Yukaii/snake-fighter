...
class GameRoom {
    ...
    createSnake() {
        const startX = Math.floor(Math.random() * (GAME_CONFIG.CANVAS_WIDTH / GAME_CONFIG.GRID_SIZE - 3)) + 1;
        const startY = Math.floor(Math.random() * (GAME_CONFIG.CANVAS_HEIGHT / GAME_CONFIG.GRID_SIZE - 3)) + 1;
...
io.on('connection', (socket) => {
    ...
    socket.on('create-room', (playerData) => {
        ...
        socket.join(roomId);
        socket.emit('room-created', { roomId, room: room.getRoomData() });
...
server.listen(PORT, () => {
    ...
    console.log(`Game Config: Max Snake Length: ${GAME_CONFIG.MAX_SNAKE_LENGTH}, Game Speed: ${GAME_CONFIG.GAME_SPEED}ms`);
});